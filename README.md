...
- In the paper, we tested some Enterprise Access Points to see if they _might_ be vulnerable when enabling their main "block peer-to-peer communication" setting (see Section VII.E in the paper for details). Whether a specific network configuration in practice is vulnerable also depends on other configuration aspects. In other words, if an Enterprise device is listed as affected in the paper, this does not mean it is inherently vulnerable. Instead, it means its basic client isolation configuration _might_ be vulnerable, making it important to test the overall client isolation behaviour of the network, as this may also depend on other configuration parameters of the tested Enterprise device. Overall, our work shows that securely configuring client isolation can be surprisingly tedious, making real-world tests important.
